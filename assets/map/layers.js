...
export function installMapSourcesAndLayers({ map, addPmtilesSource, COLORS, ZONING_FILL_COLOR, contourDemSource, saleGeoJson, salePointGeoJson, unmappedGeoJson }) {
  ...
  map.addLayer({ id: 'huc12-lines', type: 'line', source: 'huc12', 'source-layer': 'huc12', minzoom: 6, layout: { visibility: 'none' }, paint: { 'line-color': '#44d7e8', 'line-opacity': 0.9, 'line-width': ['interpolate', ['linear'], ['zoom'], 7, 1, 12, 2.2] } });
  map.addLayer({
    id: 'huc12-labels', type: 'symbol', source: 'huc12', 'source-layer': 'huc12', minzoom: 6,
  ...
  map.addLayer({
    id: 'wetlands', type: 'raster', source: 'wetlands-wms', minzoom: 6,
    layout: { visibility: 'none' }, paint: { 'raster-opacity': 0.62, 'raster-fade-duration': 0 }
  });
  map.addLayer({ id: 'cell-att', type: 'fill', source: 'cell_att', 'source-layer': 'cell_att', minzoom: 6, layout: { visibility: 'none' }, paint: { 'fill-color': '#00c5ff', 'fill-opacity': 0.12 } });
  map.addLayer({ id: 'cell-tmobile', type: 'fill', source: 'cell_tmobile', 'source-layer': 'cell_tmobile', minzoom: 6, layout: { visibility: 'none' }, paint: { 'fill-color': '#ff00c5', 'fill-opacity': 0.12 } });
  map.addLayer({ id: 'cell-verizon', type: 'fill', source: 'cell_verizon', 'source-layer': 'cell_verizon', minzoom: 6, layout: { visibility: 'none' }, paint: { 'fill-color': '#ef3030', 'fill-opacity': 0.12 } });
  map.addLayer({ id: 'pct-casing', type: 'line', source: 'pct', 'source-layer': 'pct', minzoom: 6, layout: { visibility: 'none', 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': 'rgba(37, 25, 8, 0.92)', 'line-width': ['interpolate', ['linear'], ['zoom'], 7, 3.2, 14, 7] } });
  map.addLayer({ id: 'pct', type: 'line', source: 'pct', 'source-layer': 'pct', minzoom: 6, layout: { visibility: 'none', 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': '#e69800', 'line-width': ['interpolate', ['linear'], ['zoom'], 7, 1.7, 14, 4], 'line-opacity': 0.98 } });
  map.addLayer({ id: 'pct-markers', type: 'circle', source: 'pct_markers', 'source-layer': 'pct_markers', minzoom: 6, layout: { visibility: 'none' }, paint: { 'circle-radius': ['interpolate', ['linear'], ['zoom'], 10, 2.5, 14, 4], 'circle-color': '#1d1609', 'circle-stroke-color': '#e69800', 'circle-stroke-width': 1.5 } });
  map.addLayer({
    id: 'groundwater-basins-fill', type: 'fill', source: 'groundwater_basins', 'source-layer': 'groundwater_basins', minzoom: 6,
    layout: { visibility: 'none' }, paint: { 'fill-color': '#9e6eea', 'fill-opacity': 0.12 }
  });
  map.addLayer({
    id: 'groundwater-basins-lines', type: 'line', source: 'groundwater_basins', 'source-layer': 'groundwater_basins', minzoom: 6,
  ...
  map.addLayer({
    id: 'groundwater-basins-labels', type: 'symbol', source: 'groundwater_basins', 'source-layer': 'groundwater_basins', minzoom: 6,
  ...
  addWellSymbols(map);
  map.addLayer({
    id: 'groundwater-wells', type: 'symbol', source: 'groundwater_wells', 'source-layer': 'groundwater_wells', minzoom: 6,
    layout: {
      visibility: 'none',
  ...
  map.addLayer({
    id: 'geology', type: 'fill', source: 'geology', 'source-layer': 'geology', minzoom: 6,
  ...
  map.addLayer({
    id: 'rcra-sites', type: 'circle', source: 'rcra_sites', 'source-layer': 'rcra_sites', minzoom: 6, layout: { visibility: 'none' },
    paint: {
  ...
  map.addLayer({ id: 'wildfire-perimeters-fill', type: 'fill', source: 'wildfire_perimeters', 'source-layer': 'wildfire_perimeters', minzoom: 6, layout: { visibility: 'none' }, paint: { 'fill-color': ['interpolate', ['linear'], ['to-number', ['get', 'YEAR_'], 1900], 1900, '#f5d66b', 2000, '#ee9c44', 2020, '#d95032'], 'fill-opacity': 0.16 } });
  map.addLayer({ id: 'wildfire-perimeters-lines', type: 'line', source: 'wildfire_perimeters', 'source-layer': 'wildfire_perimeters', minzoom: 6, layout: { visibility: 'none', 'line-join': 'round' }, paint: { 'line-color': ['interpolate', ['linear'], ['to-number', ['get', 'YEAR_'], 1900], 1900, '#c8a52f', 2000, '#d66f25', 2020, '#b83222'], 'line-width': ['interpolate', ['linear'], ['zoom'], 7, 1, 13, 2.2], 'line-opacity': 0.92 } });
  map.addLayer({ id: 'recent-wildfire-perimeters-fill', type: 'fill', source: 'recent_wildfire_perimeters', 'source-layer': 'recent_wildfire_perimeters', minzoom: 6, layout: { visibility: 'none' }, paint: { 'fill-color': '#d95032', 'fill-opacity': 0.2 } });
  ...
  map.addLayer({
    id: 'waterways-casing', type: 'line', source: 'waterways', 'source-layer': 'waterways', minzoom: 6,
  ...
  map.addLayer({
    id: 'waterways', type: 'line', source: 'waterways', 'source-layer': 'waterways', minzoom: 6,
    paint: {
  ...
  map.addLayer({
    id: 'waterbodies', type: 'fill', source: 'waterbodies', 'source-layer': 'waterbodies', minzoom: 6,
    paint: { 'fill-color': '#4aafff', 'fill-opacity': 0.58, 'fill-outline-color': '#167cc2' }
  });
  map.addLayer({
    id: 'waterbody-labels', type: 'symbol', source: 'waterbodies', 'source-layer': 'waterbodies', minzoom: 6,
  ...
  map.addLayer({
    id: 'summits', type: 'symbol', source: 'summits', 'source-layer': 'summits', minzoom: 6,
    layout: {
  ...
  addSpringSymbol(map);
  map.addLayer({
    id: 'springs', type: 'symbol', source: 'springs', 'source-layer': 'springs', minzoom: 6,
  ...
  map.addLayer({
    id: 'waterway-labels', type: 'symbol', source: 'waterways', 'source-layer': 'waterways', minzoom: 6,
    filter: ['all', ['has', 'gnis_name'], ['!=', ['get', 'gnis_name'], '']],
    layout: {
  ...
  map.addLayer({ id: 'forest-roads', type: 'line', source: 'forest_roads', 'source-layer': 'forest_roads', minzoom: 6, layout: { 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': '#c7a85b', 'line-width': ['interpolate', ['linear'], ['zoom'], 9, 0.55, 15, 2.2], 'line-opacity': 0.72 } });
  map.addLayer({
    id: 'forest-road-labels', type: 'symbol', source: 'forest_roads', 'source-layer': 'forest_roads', minzoom: 6,
  ...
  map.addLayer({ id: 'roads', type: 'line', source: 'roads', 'source-layer': 'roads', minzoom: 6, paint: { 'line-color': '#f8d37c', 'line-width': ['interpolate', ['linear'], ['zoom'], 9, 0.7, 15, 3], 'line-opacity': 0.9 } });
  map.addLayer({
    id: 'road-labels',
    type: 'symbol',
    source: 'roads',
  ...
  map.addLayer({
    id: 'railroad-casing', type: 'line', source: 'railroads', 'source-layer': 'railroads', minzoom: 6,
    layout: { 'line-cap': 'round', 'line-join': 'round' },
    paint: { 'line-color': 'rgba(255, 255, 255, 0.9)', 'line-width': ['interpolate', ['linear'], ['zoom'], 7, 2.8, 14, 6.5] }
  });
  map.addLayer({
    id: 'railroads', type: 'line', source: 'railroads', 'source-layer': 'railroads', minzoom: 6,
    layout: { 'line-cap': 'round', 'line-join': 'round' },
    paint: { 'line-color': '#e53935', 'line-width': ['interpolate', ['linear'], ['zoom'], 7, 1.5, 14, 3.5] }
  });
  map.addLayer({
    id: 'railroad-ties', type: 'line', source: 'railroads', 'source-layer': 'railroads', minzoom: 6,
    layout: { 'line-cap': 'butt', 'line-join': 'round' },
    paint: { 'line-color': '#681616', 'line-width': ['interpolate', ['linear'], ['zoom'], 10, 1.4, 14, 2.2], 'line-dasharray': [0.5, 2.5] }
  });
  map.addLayer({
    id: 'railroad-labels', type: 'symbol', source: 'railroads', 'source-layer': 'railroads', minzoom: 6,
