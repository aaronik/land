...
export function createParcelDetails({ detailsElement, directionsOrigin, featureCenter, getApnIndex, getSaleData, wildfirePerimetersQueryUrl, recentWildfirePerimetersQueryUrl, parcelsQueryUrl, addressPointsQueryUrl, onParcelQuest, onAdjustParcel, isParcelAdjusted, onClose }) {
  ...
  const recordCard = (record, extraLink = '') => {
    ...
    if (record.kind === 'tax-delinquent') return `<article class="record public tax-delinquent"><strong>Tax-defaulted property — not an auction listing</strong><p>${money(record.redemptionAmount)} current defaulted-tax balance · Payment plan: ${escapeHtml(record.paymentPlan || 'Not reported')}</p><p>${escapeHtml(record.source || '')}</p>${record.sourceUrl ? `<a href="${escapeHtml(record.sourceUrl)}" target="_blank" rel="noopener">Official current tax record ↗</a>` : ''}<p class="source-note">Current status was checked during the monthly refresh. It can change; verify with Siskiyou County.</p>${extraLink}</article>`;
