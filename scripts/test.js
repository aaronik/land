...
const required = ['index.html', 'assets/style.css', 'assets/app.source.js', 'data/parcels.json', 'data/generated/flood.pmtiles', 'data/generated/soils.pmtiles', 'data/generated/farmland.pmtiles', 'data/generated/rcra_sites.pmtiles', 'data/generated/huc12.pmtiles', 'data/generated/railroads.pmtiles', 'scripts/refresh-data.js'];
...
if (!failures.length) {
  ...
  if (!parcelDetails.includes("ZONING_CODE_URL = 'https://library.municode.com/ca/siskiyou_county/codes/code_of_ordinances?nodeId=TIT10PLZO'") || !parcelDetails.includes('const ZONING_EXPLANATIONS = [') || !parcelDetails.includes('data-zoning-explainer')) failures.push('Parcel zoning does not provide a Siskiyou County zoning explainer');
  if (!parcelDetails.includes('Prime Agriculture (AG-1)') || !parcelDetails.includes('Non-Prime Agriculture (AG-2)') || !parcelDetails.includes('minimum parcel size in acres')) failures.push('Agricultural zoning explanations do not distinguish AG-1 from AG-2 or explain B combining districts');
  if (!parcelDetails.includes('openZoningExplainer') || !parcelDetails.includes("dialog.className = 'zoning-explainer-dialog'") || !parcelDetails.includes("target.querySelector('[data-zoning-explainer]')?.addEventListener('click'")) failures.push('Parcel zoning explainer popup is missing');
  if (!parcelDetails.includes('municipalZoningExplanation(zoning)') || !parcelDetails.includes('ecode360.com/51184723') || !parcelDetails.includes('zoneclass: feature.properties.zoneclass') || !parcelDetails.includes('openZoningExplainer(zoning)')) failures.push('Municipal zoning explainer does not use jurisdiction-specific source metadata and code');
  if (!parcelDetails.includes("target.innerHTML = zoning ?") || parcelDetails.includes('Land use ${escapeHtml(p.LandUse1)')) failures.push('Parcel details do not replace numeric land use with an interactive zoning label');
  const zoningExpressionText = app.slice(app.indexOf('const ZONING_FILL_COLOR = '), app.indexOf('const PARCELQUEST_URL'));
  const zoningExpression = Function(zoningExpressionText.replace('const ZONING_FILL_COLOR =', 'return').replace(/;\s*$/, ''))();
  const zoningValidation = require('@maplibre/maplibre-gl-style-spec').createExpression(zoningExpression, { type: 'color' });
  if (zoningValidation.result !== 'success') failures.push(`Zoning fill expression invalid: ${zoningValidation.value.map(error => error.message).join(', ')}`);
  if (!mapLayers.includes("addPmtilesSource('zoning')") || !mapLayers.includes("map.addSource('municipal_zoning', { type: 'geojson'") || !mapLayers.includes("id: 'municipal-zoning-fill'")) failures.push('Municipal zoning map overlay is missing');
  const layerConfig = fs.readFileSync(path.join(root, 'scripts/layers.js'), 'utf8');
  const layerDownload = fs.readFileSync(path.join(root, 'scripts/download-layers.js'), 'utf8');
  const layerTiles = fs.readFileSync(path.join(root, 'scripts/make-pmtiles.js'), 'utf8');
  if (!layerConfig.includes('municipal_zoning') || !layerConfig.includes("'Mount Shasta'")) failures.push('Municipal zoning source configuration is missing');
  if (!layerDownload.includes('fetchMunicipalZoning') || !layerDownload.includes('municipal_zoning')) failures.push('Municipal zoning download is missing');
  if (!layerTiles.includes("municipal_zoning: { min: 6")) failures.push('Municipal zoning tiles are not built');
  if (!app.includes("'C-1', 'C-2'")) failures.push('Municipal zoning categories are not styled');
  if (!html.includes('City and town districts are interwoven where mapped')) failures.push('Municipal zoning coverage note is missing');
